 ...
 //Create fonts
 surface.CreateFont("LuapadEditor2", {
 	font = "Courier New",
 	size = 16,
 	weight = 400
 })
 surface.CreateFont("LuapadEditor2_Bold", {
 	font = "Courier New",
 	size = 16,
 	weight = 800
 })
 ...
 function luapad.EditorPanel:PaintLine(row)
	...
	draw.SimpleText(tostring(row), "LuapadEditor2", width * 3, (row - self.Scroll[1]) * height, Color(128, 128, 128, 255), TEXT_ALIGN_RIGHT)
		...
		if(offset < 0) then
			if(string.len(cell[1]) > -offset) then
				...
				if(cell[2][2]) then
					...
					draw.SimpleText(line, "LuapadEditor2", width * 3 + 6, (row - self.Scroll[1]) * height, cell[2][1])
				end
			else
			...
			if(cell[2][2]) then
				...
				draw.SimpleText(cell[1], "LuapadEditor2", offset * width + width * 3 + 6, (row - self.Scroll[1]) * height, cell[2][1])
			end
