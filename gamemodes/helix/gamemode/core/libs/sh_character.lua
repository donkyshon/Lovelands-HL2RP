...
do
	if (SERVER) then
		...
		net.Receive("ixCharacterCreate", function(length, client)
			...
			print(4)
			PrintTable(payload)
			payload.phys = payload.phys or {}
			payload.physData = payload.physData or {}

			payload.physData[1] = math.Clamp(payload.phys[1], 18, 40)
			payload.physData[2] = math.Clamp(payload.phys[2], 150, 195)
			payload.physData[3] = math.Clamp(payload.phys[3], 1, 6)
			payload.physData[4] = math.Clamp(payload.phys[4], 1, 9)

			payload.physData[5] = payload.physData[5] or {}
			payload.physData[5][1] = math.Clamp(payload.phys[5][1], 1, 4)
			payload.physData[5][2] = math.Clamp(payload.phys[5][2], 1, 5)
			payload.physData[6] = math.Clamp(payload.phys[6], 1, 4)
			payload.physData[7] = math.Clamp(payload.phys[7], 1, 2)
			payload.physData[8] = Names[payload.physData[7]][payload.physData[6]]
