...
function GM:LoadFonts(font, genericFont)
	...
	surface.CreateFont("ixSmallFont2", {
		font = "Blender Pro Bold",
		size = ScreenScaleMin(24 / 4),
		extended = true,
		weight = 500
	})
...
function GM:PopulateCharacterInfo(client, character, container)
	local eye, eyeClr = character:GetEyeColor()
	local hair, hairClr = character:GetHairColor()
	local gender, genderClr = character:GetGenderName()

	local text = L("phys_desc", L(character:GetBodyShape()):utf8upper(), character:GetHeight():utf8upper(), character:GetAge(), L(eye):utf8upper(),L(hair):utf8upper(),L(gender):utf8upper())

	local description = container:AddRow("description2")
		description:SetText(text)
		description:SetFont("ixSmallFont2")
