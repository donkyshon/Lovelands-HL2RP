...
function CheckJump(ply, mv, velocity)
	...
	if ply:GetNWFloat("jumpstamina") > 0 then
		local flRatio = ( STAMINA_MAX - ( ( ply:GetNWFloat("jumpstamina") / 1000 ) * STAMINA_RECOVER_RATE ) ) / STAMINA_MAX
		velocity.z = velocity.z * flRatio
	end
	ply:SetNWFloat("jumpstamina", (STAMINA_COST_JUMP / STAMINA_RECOVER_RATE ) * 1000)
